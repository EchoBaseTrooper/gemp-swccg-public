...
public class GameStatisticsProcessor implements InGameStatisticsListener {
	...
	public void writePileCounts(final GameState gameState,boolean gameComplete) {
		...
		int sequence = lightTurn+darkTurn;
		...
		_pileCountByTurnDAO.addPileCounts(_currentGameID, sequence, Math.max(darkTurn, lightTurn), side, darkHand, darkReserveDeck, darkForcePile, darkUsedPile, darkLostPile, darkOutOfPlay, lightHand, lightReserveDeck, lightForcePile, lightUsedPile, lightLostPile, lightOutOfPlay);
	...
	@Override
	public void writeActivationTotals(GameState gameState) {
		String dark = gameState.getPlayer(Side.DARK);
		String light = gameState.getPlayer(Side.LIGHT);
